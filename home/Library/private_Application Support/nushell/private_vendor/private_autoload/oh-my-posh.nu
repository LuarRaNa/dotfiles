...
let _omp_config: string = ($env.HOME | path join ".config/ohmyposh/config.toml")
...
def --wrapped _omp_get_prompt [
    ...
    let cmd_duration = ($env.CMD_DURATION_MS? | default '0823')
    if $cmd_duration != '0823' {
        $execution_time = $cmd_duration
        $no_status = false
    }
...
$env.PROMPT_COMMAND = {||
    ...
    if $nu.history-enabled {
        let last_command = (try { history | last 1 | get 0.command } catch { null })
        $clear = ($last_command == null) or ($last_command == "clear")
    }
